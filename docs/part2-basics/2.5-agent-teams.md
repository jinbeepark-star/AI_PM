...
> **4.7 + PM 운영 현실**: 4.7 은 기본적으로 적은 수의 subagent 만 띄우고, 단일 세션의 적응적 추론을 더 신뢰합니다. PM 운영 측면에서는 **사용자 측 병렬 세션**(3–6개) 이 새로운 정상치 — 같은 사람이 git worktree (또는 cloud worktree, 챕터 3.6) 로 동시에 여러 작업을 돌리는 패턴입니다. Stop 훅(2.7) 알림으로 어느 세션이 끝났는지 즉시 압니다. 자세한 운영 룰은 챕터 3.6 (Claude Code on the web) 의 "Cloud worktree" 절 참조.
